...
import { getRoomsWithBeds, getComplaints, getNotices, getRents, Room, Complaint, Notice, Rent } from '@/lib/store';
  ...
  useEffect(() => {
    ...
    if (isAdmin) {
      setStats({
        ...
        occupiedBeds: allBeds.filter(b => b.is_occupied).length,
        totalBeds: allBeds.length,
        openComplaints: complaints.filter(c => c.status !== 'RESOLVED').length,
        pendingRent: rents.filter(r => r.status === 'PENDING').length,
      });
      setRecentComplaints(complaints.slice(0, 5));
      setRecentNotices(notices.slice(0, 3));
    } else {
      setRecentNotices(notices.slice(0, 5));
      setMyComplaints(complaints.filter(c => c.student_id === profile?.id).length);
      setMyPendingRent(rents.filter(r => r.student_id === profile?.id && r.status === 'PENDING').reduce((s, r) => s + r.amount, 0));
    }
    setLoading(false);
  }, [isAdmin, profile]);
      ...
      <>
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-3 mb-10">
          {studentStats.map((stat, i) => (
            <div key={stat.name} className={`${stat.cardClass} rounded-2xl p-5 transition-all duration-300 hover:scale-[1.02] animate-fade-in-up opacity-0`}
              style={{ animationDelay: `${i * 0.08}s`, animationFillMode: 'forwards' }}>
              <div className="flex items-center justify-between">
                <div><p className="text-xs font-medium text-slate-400 uppercase tracking-wider">{stat.name}</p><p className="text-2xl font-bold text-slate-100 mt-2">{stat.value}</p></div>
                <div className={`${stat.iconColor} opacity-60`}><stat.icon className="h-10 w-10" strokeWidth={1.5} /></div>
              </div>
            </div>
          ))}
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
          <div className="glass-card p-6 animate-fade-in-up opacity-0" style={{ animationDelay: '0.3s', animationFillMode: 'forwards' }}>
            <div className="flex items-center gap-2 mb-5"><Zap className="h-5 w-5 text-cyan-400" /><h2 className="text-lg font-bold text-slate-100">Quick Actions</h2></div>
            <div className="space-y-3">
              {studentActions.map((action) => (
                <Link key={action.name} href={action.href}
                  ...
                  <div><p className="text-sm font-semibold text-slate-200">{action.name}</p><p className="text-xs text-slate-500">{action.desc}</p></div>
          ...
          <div className="mb-10 animate-fade-in-up">
            <div className="flex items-center gap-3 mb-2">
              <div className="h-10 w-10 rounded-xl flex items-center justify-center"
                style={{ background: isAdmin ? 'linear-gradient(135deg, rgba(99,102,241,0.2), rgba(139,92,246,0.15))' : 'linear-gradient(135deg, rgba(6,182,212,0.2), rgba(59,130,246,0.15))', border: isAdmin ? '1px solid rgba(99,102,241,0.3)' : '1px solid rgba(6,182,212,0.3)' }}>
                <Activity className="h-5 w-5" style={{ color: isAdmin ? '#818cf8' : '#22d3ee' }} />
              </div>
              <div>
                <h1 className="text-2xl font-bold text-slate-100">{isAdmin ? 'Welcome back, ' : 'Hey, '}<span className="gradient-text">{profile?.name}</span></h1>
                <p className="text-slate-400 text-sm mt-0.5">{isAdmin ? "Here's your hostel management overview" : 'Your accommodation dashboard'}</p>
              </div>
            </div>
          </div>
          {loading ? (
