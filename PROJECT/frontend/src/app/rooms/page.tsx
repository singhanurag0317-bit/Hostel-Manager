...
import { getRoomsWithBeds, addRoom, deleteRoom, addBeds, deleteBedsForRoom, updateBed, Room, Bed } from '@/lib/store';
...
export default function RoomsPage() {
  ...
  const fetchRooms = () => { setRooms(getRoomsWithBeds()); setLoading(false); };
  ...
  const handleAddRoom = (e: React.FormEvent) => {
    ...
    try {
      const room = addRoom({ room_number: newRoom.roomNumber, floor: newRoom.floor, rent_amount: newRoom.rentAmount, capacity: newRoom.totalBeds });
      addBeds(Array.from({ length: newRoom.totalBeds }, (_, i) => ({ bed_number: `${i + 1}`, room_id: room.id, is_occupied: false })));
      setShowAddModal(false); setNewRoom({ roomNumber: '', totalBeds: 4, floor: 1, rentAmount: 5000 }); fetchRooms();
    } catch (err: any) { setError(err.message || 'Failed to add room'); }
    finally { setSubmitting(false); }
  };

  const handleDeleteRoom = (roomId: string) => {
    if (!confirm('Delete this room and all its beds?')) return;
    deleteBedsForRoom(roomId); deleteRoom(roomId); fetchRooms();
  };

  const toggleBedOccupancy = (bedId: string, currentStatus: boolean) => {
    updateBed(bedId, { is_occupied: !currentStatus }); fetchRooms();
  };
                  ...
                  <div key={room.id} className="glass-card p-5 animate-fade-in-up opacity-0" style={{ animationDelay: `${i * 0.06}s`, animationFillMode: 'forwards' }}>
                    <div className="flex justify-between items-start mb-4">
                      ...
                      <div className="flex items-center gap-2">
                        <span className={`badge ${pct === 100 ? 'badge-red' : pct > 50 ? 'badge-yellow' : 'badge-green'}`}>{occupiedCount}/{totalBeds}</span>
                        {isAdmin && (
                    ...
                    <div className="mb-4">
                      <div className="flex justify-between text-xs text-slate-500 mb-1.5">
                        <span>Occupancy</span><span className="font-medium text-slate-300">{Math.round(pct)}%</span>
                      </div>
                      <div className="progress-bar-container">
                        <div className={`progress-bar-fill ${barColor}`} style={{ width: `${pct}%` }} />
                      </div>
                    </div>
                    <p className="text-xs text-slate-400 mb-4"><span className="text-slate-500">Rent:</span> <span className="text-emerald-400 font-semibold">₹{room.rent_amount}/mo</span></p>
